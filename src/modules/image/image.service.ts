...
import { NotFoundtMessage, PublicMessage } from 'src/common/enums/message.enum';
...
export class ImageService {
  ...
  async create(imageDto: ImageDto, image: MulterFile) {
    const userId = (this.req.user as any).id;
  ...
  async findOne(id: number): Promise<ImageEntity> {
    ...
    if (!image) throw new NotFoundException(NotFoundtMessage.NotFound);
